import { InstallmentWithTotalInstallments, InvoiceDetails } from "@/@types/prisma-customTypes";
...
export class PayInvoice{
	...
	async confirmInvoicePayment(userId: string, invoiceDetails: InvoiceDetails[]){
		...
		const totalInstallmentsOnInvoice = invoiceDetails[0].totalInstallmentsOnInvoice;
		const totalInstallmentsPaid = invoiceDetails[0].installmentsPaid;
		
		const currentDate = new Date().getTime();
		const closingDate = new Date(invoiceDetails[0].closingDate).getTime();
	...
	async confirmFullPaymentForAPurchase(installments: InstallmentWithTotalInstallments[]){
		...
		installments.forEach((installment) => {
			if(installment.installmentNumber === installment.shopping.totalInstallments){
				shoppingId.push(installment.shoppingId);
			}
		});
	...
	async execute(userId: string, invoiceId: string, installmentsToPay: string[]){
		...
		const updateShoppings = installmentsPaid.map((installment) => installment.shoppingId);
